...
- `subdivide(mesh, args)` with `args.mode() == SubdivisionMode::RadialRandomSplit { seed, elevation_noise_range }`:
  ...
  - Produces no vertex with radius `> 1.0 + steps * elevation_noise_range.high()` after `steps` rounds. This bound **compounds with step count** and is not just `1.0 + elevation_noise_range.high()`: each round's new vertex is the midpoint of two parent vertices that may themselves already carry a prior round's displacement, so a lineage of vertices can each add up to `elevation_noise_range.high()` on top of the previous round's maximum, round after round. (The lower bound does not compound the same way, because `MIN_VERTEX_RADIUS` is a hard per-round floor, not an additive term.)
...
7. For an `ElevationNoiseRange` of low `-0.1`/high `0.1`, every vertex's radius after 2 steps is within `[0.05, 1.2]` (upper bound is `1.0 + steps * high`, not `1.0 + high` — the displacement compounds across rounds since a round's parents may already carry a prior round's displacement)
