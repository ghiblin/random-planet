Feature: Constructing SubdivisionArgs with defaults
  ...
  Scenario: Constructing SubdivisionArgs with explicit steps and mode
    ...
    When SubdivisionArgs is constructed with those steps and the UniformRedSplit mode with seed 7
    Then the SubdivisionArgs has 5 steps
    And the SubdivisionArgs has the UniformRedSplit mode with seed 7

  Scenario: Omitting steps defaults to 3
    When SubdivisionArgs is constructed with no steps and the UniformRedSplit mode with seed 7
